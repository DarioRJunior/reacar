...
export const List = ({ cars: carsInitial }: ListProps) => {
    ...
    return (
        <div className="w-full flex justify-center bg-gray-100 min-h-screen">
            <div className="flex flex-col w-full m-4 md:w-2/3 xl:1/2">
                <SearchForm onSubmit={async (term) => {
                ...
                <h2 className="text-3xl font-medium my-4 text-gray-800">Itens recomendados</h2>

                {cars.length === 0 && (
                    <h2 className="text-2xl font-medium my-4 text-gray-800">Ops, não há itens disponíveis</h2>
                )}

                {cars.map((car) => (
                    <div className="w-full shadow p-3 mb-5 rounded hover:shadow-xl transition ease-out duration-1000"
                        key={car.id}
                    >
                        <div className="w-full h-96 relative">
                            <Image src={car.image} alt={car.make} layout="fill" className="rounded" objectFit="cover" />
                        </div>
                        ...
                        <p className="font-bold text-gray-800 py-3 text-lg">
                            {Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(car.price)}
                        </p>

                        <div className="flex justify-between">
                            <CarItemSpecification text={car.fuel} icon={<FiTool />} />
                            <CarItemSpecification text={car.year} icon={<FiClock />} />
                            <CarItemSpecification text={car.mileage.toString()} icon={<FiMapPin />} />
                        </div>
                    </div>
                ))}
            </div>
